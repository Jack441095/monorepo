...
def _nite_dsp_root() -> Path:
    ...
    shadow_adapters -- see any of them for why a fixed parent-index
    assumption is the wrong shape here.
    """
    import os
    ...
    for candidate in list(here.parents)[:6]:
        if (candidate / TRUTH_SHEET_RELATIVE_PATH).exists():
            return candidate
    # Fallback depth for this module's location today: thursday/ops/
    # beta_invite_ops.py -> parents[3] is NITE_DSP root.
