...
CODEBASE_MAP_PATH = Path(__file__).resolve().parent.parent / "docs" / "CODEBASE_MAP.md"
...
def decide(
    ...
    timeout = int(os.environ.get("AUDIO_TOO_LLM_TIMEOUT", "10"))
    ...
    content = ""
    try:
        result = DEFAULT_LLM.generate(messages, timeout=timeout, response_schema=response_schema)
        ...
        try:
            ...
            return BrainDecision(type="abstain", abstract=f"LLM call failed: {first_err}", steps=[], confidence="low")
