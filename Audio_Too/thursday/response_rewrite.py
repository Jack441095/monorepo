...
Reuses audio_too.model_runtime.DEFAULT_LLM -- the same provider
thursday/brain.py already calls successfully in this environment (local-first
via Ollama, auto-resolving without needing AUDIO_TOO_LLM_ENABLED set at all --
see EnvGatedProvider). studio/kenn/kenn/llm/llm_rewrite.py was considered
instead (it already has a plain-text chat_completion() with caching/retry),
but its readiness gate requires AUDIO_TOO_LLM_ENABLED specifically, which is
unset in Jack's .env even though THURSDAY_BRAIN_ENABLED=1 already works --
that mismatch would have made this feature silently no-op. DEFAULT_LLM.generate()
takes json_mode=False here so it doesn't force JSON-object output the way
brain.py's structured decisions need.
"""
...
def rewrite(question: str, raw_text: str, service_name: str) -> str | None:
    ...
    try:
        result = DEFAULT_LLM.generate(messages, timeout=10, json_mode=False)
    except Exception:
