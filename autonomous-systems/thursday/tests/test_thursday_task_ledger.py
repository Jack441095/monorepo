...
# ── save failures are loud (2026-09-18 audit fix) ─────────────────────────


def test_create_task_raises_loudly_when_ledger_unwritable(monkeypatch, tmp_path):
    """Failing-first for silent-save swallowing: the old `_save` caught
    OSError and passed, so the caller believed a task persisted when
    nothing was stored. Now raises LedgerWriteError (an OSError
    subclass, so existing handlers keep working)."""
    blocker = tmp_path / "blocker"
    blocker.write_text("a file, not a directory")
    monkeypatch.setattr(tl, "TASK_LEDGER_FILE", blocker / "task_ledger.json")
    with pytest.raises(tl.LedgerWriteError):
        tl.create_task("marketing", "This task is never stored.")
    assert not (blocker / "task_ledger.json").exists()


def test_update_task_raises_loudly_when_ledger_unwritable(monkeypatch, tmp_path):
    import os
    store = tmp_path / "store"
    store.mkdir()
    monkeypatch.setattr(tl, "TASK_LEDGER_FILE", store / "task_ledger.json")
    t = tl.create_task("marketing", "Stored fine.")
    os.chmod(store, 0o555)  # readable (load works) but not writable (save fails)
    try:
        with pytest.raises(tl.LedgerWriteError):
...
def test_ledger_write_error_is_an_oserror():
    assert issubclass(tl.LedgerWriteError, OSError)
