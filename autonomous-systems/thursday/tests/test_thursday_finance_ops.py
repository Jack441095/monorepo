...
def test_business_knowledge_env_override_wins(monkeypatch, tmp_path):
    import json as _json
    fixture = tmp_path / "bk.json"
    fixture.write_text(_json.dumps({"offerings": [
        {"id": "env_service", "name": "Env Service", "price_from_gbp": 11,
         "turnaround": "1 day"},
    ], "faqs": []}))
    monkeypatch.setenv("THURSDAY_BUSINESS_KNOWLEDGE_FILE", str(fixture))
    offerings = fo.get_offerings()
    assert len(offerings) == 1 and offerings[0]["name"] == "Env Service"


def test_business_knowledge_env_override_missing_file_is_honest(monkeypatch, tmp_path):
    monkeypatch.setenv("THURSDAY_BUSINESS_KNOWLEDGE_FILE",
