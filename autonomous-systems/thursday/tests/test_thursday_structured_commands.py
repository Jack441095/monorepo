...
def test_dispatches_find_customers(monkeypatch):
    monkeypatch.delenv("TAVILY_API_KEY", raising=False)
    result = try_dispatch("find customers: UK music students")
    assert result is not None
    name, response = result
    assert name == "find_customers"
    assert "UK music students" in response


def test_dispatches_web_search(monkeypatch):
    monkeypatch.delenv("TAVILY_API_KEY", raising=False)
    result = try_dispatch("web search: competitor pricing")
    assert result is not None
    name, response = result
    assert name == "web_search"
    assert "competitor pricing" in response
