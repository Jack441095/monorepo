...
try:
    ...
    # Deferred, not vendored (see docs/EXTRACTION_COUPLING.md). ROOT now
    # finds the real Audio_Too/ even from the standalone extraction (see
    # thursday.repo_root's docstring for why a fixed parent.parent never
    # could), so this branch only fires if Audio_Too genuinely isn't on
    # disk at all. Fails CLOSED (deny) rather than raising or defaulting
    # to allowed: the one call site (record_audio(), below) gates real
    # microphone capture, a genuine mutating action, on this check -- an
    # unavailable policy module must never silently grant permission.
    # Same pattern as scheduling.py.
