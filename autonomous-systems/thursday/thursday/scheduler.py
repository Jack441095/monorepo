...
# Ensure business app is importable for db (Audio_Too's own directory
# layout, found via audio_too_root()'s upward search -- see that
# function's docstring for why a fixed parent.parent can't reach it from
# the standalone extraction, a sibling of Audio_Too rather than an
# ancestor).
if str(audio_too_root() / "business" / "app") not in sys.path:
    sys.path.insert(0, str(audio_too_root() / "business" / "app"))

try:
    from db import connect, now
except ImportError:
    # Deferred, not vendored: `connect`/`now` are Audio_Too's real
    # scheduling-DB primitives, a genuine cross-repo integration point,
    # not something safe to fake with stub logic (see docs/
    # EXTRACTION_COUPLING.md). Reached only if Audio_Too genuinely isn't
    # on disk at all -- the module itself must still *import* cleanly
    # standalone -- callers that actually invoke scheduler
    # functions outside Audio_Too get a clear, typed failure instead of
    # this whole module (and everything that imports it transitively,
    # including thursday.orchestrator and thursday.registry) refusing to
    # import at all.
    class SchedulerDatabaseUnavailable(RuntimeError):
        pass

    def connect(*_args, **_kwargs):  # type: ignore[no-redef]
        raise SchedulerDatabaseUnavailable(
            "thursday.scheduler requires Audio_Too's business/app/db "
            "module, not available standalone"
        )

    def now(*_args, **_kwargs):  # type: ignore[no-redef]
        raise SchedulerDatabaseUnavailable(
            "thursday.scheduler requires Audio_Too's business/app/db "
            "module, not available standalone"
        )
...
def dispatch_agent(agent_name: str, method_name: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    agents_shared = audio_too_root() / "business" / "agents" / "Shared"
    if str(agents_shared) not in sys.path:
        sys.path.insert(0, str(agents_shared))


    # Load agent module
    agent_path = audio_too_root() / "business" / "agents" / agent_name
