...
class LedgerWriteError(OSError):
    """Raised when the ledger cannot be persisted. Subclasses OSError so
    existing `except OSError` handlers keep working -- but unlike the old
    silent `except OSError: pass` in _save, the caller now KNOWS the task
    was not stored (2026-09-18 audit fix: silent saves let the founder
    believe a task persisted when it hadn't)."""
    ...
    if TASK_LEDGER_FILE.exists():
        try:
            ...
            if isinstance(data, dict) and "tasks" in data:
                return data
            logger.warning("task_ledger: %s has unexpected shape; starting empty (no overwrite until next save)", TASK_LEDGER_FILE)
        except (json.JSONDecodeError, OSError) as exc:
            logger.warning("task_ledger: corrupt/unreadable %s (%s); starting empty (no overwrite until next save)", TASK_LEDGER_FILE, exc)
...
def _save(data: dict) -> None:
    try:
        ...
        logger.error("task_ledger: FAILED to persist %s (%s) -- caller notified, nothing stored",
                     TASK_LEDGER_FILE, exc)
        raise LedgerWriteError(f"could not persist task ledger to {TASK_LEDGER_FILE}: {exc}") from exc
