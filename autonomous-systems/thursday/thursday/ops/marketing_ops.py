...
_LEVELS = {"High": 3, "Medium": 2, "Low": 1}

# Standing suggestions (not data): the highest-leverage next marketing
# moves for a solo founder, each traceable to a real source. Impact ×
# effort ranks them; existing on-file plans always list first as DATA.
_SUGGESTIONS = [
    {"item": "Instrument waitlist-submit + pricing-view events (Plausible custom events)",
     "impact": "High", "effort": "Low",
     "source": "advisory_ops.what_to_instrument: launch decisions are blind on conversion"},
    {"item": "Collect review + referral on every completed job (sop policy)",
     "impact": "High", "effort": "Low",
     "source": "thursday-sops: review + referral requested on every completed job"},
    {"item": "State the free beta (0.2.0) on the beta page to defuse price questions",
     "impact": "Medium", "effort": "Low",
     "source": "truth sheet Version 0.2.0 + support FAQ quote flow"},
    {"item": "Publish from the corpus (SOPs + truth sheet + 38 pages), not from scratch",
     "impact": "Medium", "effort": "Low",
     "source": "doc-search corpus eval 30/30 cited"},
    {"item": "Pick ONE channel until £1k MRR; log per-channel leads in the ledger",
     "impact": "High", "effort": "Medium",
     "source": "ops plan Phase 5 trade engine rule"},
    {"item": "Keep product/service price messaging separate (£3 app vs £45-180 services)",
     "impact": "Medium", "effort": "Low",
     "source": "website pricing check scope note"},
]


def backlog() -> list[dict]:
    """Prioritized marketing backlog: on-file plans (DATA) first, then
    standing suggestions (SUGGESTION, impact×effort ranked). Suggestions
    are labelled as such -- they are Thursday's standing recommendations,
    not measured results."""
    items = []
    for plan in list_marketing_plans():
        items.append({"item": plan.objective, "impact": "—", "effort": "—",
                      "score": None, "kind": "DATA", "source": f"marketing plan {plan.plan_id}"})
    for s in _SUGGESTIONS:
        items.append({**s, "score": _LEVELS[s["impact"]] / _LEVELS[s["effort"]],
                      "kind": "SUGGESTION"})
    items.sort(key=lambda i: (i["score"] is not None, -(i["score"] or 0), i["item"]))
    return items


def render_backlog(items: list[dict] | None = None) -> str:
    items = backlog() if items is None else items
    lines = ["MARKETING BACKLOG (impact × effort; DATA first, then SUGGESTION)", ""]
    if not items:
        lines.append("  Empty -- no plans on file. Suggestions below still apply.")
    for i, item in enumerate(items, 1):
        score = f"{item['score']:.1f}" if item.get("score") is not None else "n/a (real plan)"
        lines.append(f"  {i}. [{item['kind']} {score}] {item['item']}")
        lines.append(f"     Why: {item['source']}")
    return "\n".join(lines)
