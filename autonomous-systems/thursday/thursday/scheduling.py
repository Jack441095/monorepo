...
_ROOT = audio_too_root()
if str(_ROOT / "scripts") not in sys.path:
    sys.path.insert(0, str(_ROOT / "scripts"))
try:
    from action_policy import action_allowed, action_denied_message  # noqa: E402
except ImportError:
    # Deferred, not vendored (see docs/EXTRACTION_COUPLING.md). _ROOT now
    # finds the real Audio_Too/ via repo_root.audio_too_root() even when
    # this module runs from the standalone extraction (a sibling of
    # Audio_Too, not an ancestor -- see that module's docstring), so this
    # branch is only reached if Audio_Too genuinely isn't present on disk
    # at all (e.g. a bare checkout of just this repo). Fails CLOSED (deny)
    # rather than raising or defaulting to allowed: the one call site
    # (line ~520 below) gates desktop automation, a real mutating action,
    # on this check -- an unavailable policy module must never silently
    # grant permission.
    def action_allowed(_action: str) -> bool:  # noqa: E302
        return False

    def action_denied_message(_action: str) -> str:  # noqa: E302
        return (
            "action_policy module unavailable (Audio_Too not found on "
            "disk) -- denied by default, fail-closed"
        )
