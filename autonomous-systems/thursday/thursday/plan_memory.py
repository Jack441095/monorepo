...
# Fixed vocabulary for the failure_reason column (thursday/training_export.py's
# consumer). Documented, not enforced -- an unrecognized value is still stored
# (fail-soft philosophy, same as everything else in this module), just not a
# value thursday.training_export's readers should expect to see.
_KNOWN_FAILURE_REASONS = frozenset({"llm_call_failed", "json_parse_failed", "service_exception"})

# Additive columns for the execution-feedback-loop data collection pipeline
# (route/model/confidence/latency/failure-reason/hallucination-signal). Added
# idempotently in init_db() via PRAGMA table_info + conditional ALTER TABLE,
# so an existing on-disk DB upgrades in place with no data loss.
...
def _redact_deep(obj: Any) -> Any:
    ...
    steps/executed_steps are dicts of kind/service_id/agent/task/params --
    the enum-like fields (kind/service_id/agent) are safe, but `params` and
    `task` frequently carry real user-supplied text (a message body, a
    client name), so a shallow top-level redact() would miss secrets nested
    inside them.
    """
...
def init_db() -> None:
    ...
    try:
        with _get_conn() as conn:
            ...
            # Additive migration for an existing on-disk DB created before
            # the execution-feedback-loop columns existed -- idempotent
            # (PRAGMA table_info + conditional ADD COLUMN), safe to run on
            # every process start.
            existing_cols = {row[1] for row in conn.execute("PRAGMA table_info(plan_memory)").fetchall()}
            for col, ddl_type, default in _NEW_COLUMNS:
                if col not in existing_cols:
                    try:
                        conn.execute(f"ALTER TABLE plan_memory ADD COLUMN {col} {ddl_type} DEFAULT {default}")
                    except sqlite3.OperationalError as e:
                        logger.warning(f"Failed to add plan_memory column {col!r}: {redact(str(e))}")
...
def save_plan_trace(
    ...
    *,
    ...
    query/abstract/result_summary/lesson and every string leaf inside
    steps/executed_steps are passed through thursday.redaction.redact()
    before being written -- defense in depth, since this DB is the source
    thursday.training_export.py reads from and users routinely paste real
    client names/emails/message bodies into these fields.
    """
    try:
        import datetime
        ...
        safe_abstract = redact(abstract)
        safe_result_summary = redact(result_summary) if result_summary else result_summary
        safe_lesson = redact(lesson) if lesson else lesson
        steps_str = json.dumps(_redact_deep(steps or []))
        executed_str = json.dumps(_redact_deep(executed_steps if executed_steps is not None else (steps or [])))
        ...
        with _get_conn() as conn:
            conn.execute(
                ...
                INSERT OR REPLACE INTO plan_memory (
                    ...
                    decision_type, provider, model, confidence, latency_s,
                    failure_reason, raw_step_count
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    pid, created, safe_query, safe_abstract, steps_str, executed_str,
                    status, service_id, safe_result_summary, safe_lesson, tags_str,
                    decision_type, provider, model, confidence, latency_s,
                    failure_reason, raw_step_count,
                ),
            )
            try:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?)
                    """,
                    (pid, safe_query, safe_abstract, safe_lesson or "", tags_str),
                )
            except sqlite3.OperationalError:
                pass
