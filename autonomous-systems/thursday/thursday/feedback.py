...
def record_feedback(
    *,
    ...
    response_text: str = "",
    user_text: str = "",
    plan_id: str | None = None,  # joins this feedback record to its thursday.plan_memory row
) -> dict:
    """Record a single feedback event (explicit or implicit) to the append-only log.

    Returns the stored record.

    response_text/user_text are redact()'d before truncation to the 200-char
    snippet -- previously stored raw, with no redaction applied anywhere in
    this module (this store is a source thursday.training_export.py reads
    from, and users routinely paste real client names/emails/secrets into
    chat).
    """
    record = {
        "turn_id": turn_id,
        ...
        "response_text_snippet": redact(response_text)[:200] if response_text else "",
        "user_text_snippet": redact(user_text)[:200] if user_text else "",
        "plan_id": plan_id,
