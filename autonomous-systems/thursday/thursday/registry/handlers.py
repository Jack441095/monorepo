...
def _handle_company_state(text: str, ctx: dict) -> str:
    ...
    if is_llm_enabled:
        try:
            # Goes through thursday.llm_provider (the same swappable
            # provider abstraction thursday/brain.py uses), defaulting to
            # audio_too.model_runtime.DEFAULT_LLM unchanged -- caught below
            # like every other failure here, falling back to the
            # deterministic rendering. See docs/EXTRACTION_COUPLING.md and
            # docs/THURSDAY_LLM_PROVIDER_ABSTRACTION.md.
            ...
            result = get_llm_provider().generate(messages, timeout=default_timeout())
...
_MARKETING_PLATFORMS = ("instagram", "twitter", "x", "facebook", "linkedin", "tiktok")
_CAMPAIGN_TYPE_KEYWORDS = ("launch", "promo", "promotion", "sale", "announcement")
_OUTREACH_LEAD_RE = re.compile(r"\b(?:to|for)\s+([A-Z][a-zA-Z'-]+(?:\s+[A-Z][a-zA-Z'-]+)?)\b")


def _handle_marketing_agent(api, text: str) -> str:
    """Handle marketing agent requests.

    Marketing/main.py's real functions each need two distinct fields, not
    one combined string (generate_social_post(platform, topic),
    create_campaign(type, target), draft_outreach(lead, context)) --
    previously this sent a single blob as both positional CLI args, which
    the underlying argparse/function signatures rejected outright (found
    2026-09-05: this call path was silently producing zero output for a
    different reason first -- see Marketing/main.py's __main__ guard fix --
    and would have failed here immediately afterward regardless). Fields
    that can't be reliably split out of free text get an honest, documented
    default rather than a guess dressed up as a real value.
    """
    cleaned = _clean_agent_request(text)
    lowered = cleaned.lower()
    if any(w in lowered for w in ["social", "post", "instagram"]):
        ...
        platform = next((p for p in _MARKETING_PLATFORMS if p in cmd_text.lower()), "instagram")
        topic = re.sub(rf"\b(?:on|for|to)\s+{platform}\b", "", cmd_text, flags=re.IGNORECASE).strip() or cmd_text
        return api.marketing_agent("post", platform, topic)
    elif any(w in lowered for w in ["lead"]):
        # MarketingAgent has no lead-creation capability at all (only
        # analyze_leads(), a read-only analysis) -- degrade honestly rather
        # than calling into a function that doesn't exist.
        return "I can't save new leads yet -- that capability isn't built into the marketing agent."
    elif any(w in lowered for w in ["campaign"]):
        ...
        campaign_type = next((k for k in _CAMPAIGN_TYPE_KEYWORDS if k in cmd_text.lower()), "general")
        return api.marketing_agent("campaign", campaign_type, cmd_text)
    else:
        ...
        lead_match = _OUTREACH_LEAD_RE.search(cmd_text)
        lead = lead_match.group(1) if lead_match else "the contact"
        return api.marketing_agent("outreach", lead, cmd_text)
...
def _handle_research_agent(api, text: str) -> str:
    ...
    # "suggest" was never a real subcommand of Research/main.py -- the real
    # verb is "topic" (-> research_topic(topic, depth="standard")). Found
    # 2026-09-05 alongside the marketing agent's identical class of bug.
    return api.research_agent("topic", topic)


_SPECIALIST_KEYWORDS = {
    "commercial": ("commercial specialist", "pricing specialist", "billing specialist"),
    "support": ("support specialist",),
    "documentation": ("documentation specialist", "docs specialist"),
    "qa": ("qa specialist", "quality specialist", "readiness specialist"),
    "security": ("security specialist",),
    "data": ("data specialist", "analytics specialist"),
}


def _handle_specialist_task(text: str) -> str:
    """Route an explicit "ask the <X> specialist ..." request to
    thursday.specialists.run_grounded_specialist(). Complements, not
    replaces, the existing direct deterministic lookups (_handle_pricing,
    _handle_faq, _handle_launch_tracker, _handle_beta_readiness) -- those
    stay as fast, LLM-free exact answers; this path is for a more
    open-ended ask that benefits from LLM-synthesized framing over the
    same real evidence. A security/data-directed ask still reaches
    run_grounded_specialist and gets its honest abstain message, not
    silently dropped here.
    """
    import uuid

    from thursday.specialists import REGISTRY, run_grounded_specialist
    from thursday.orchestration_models import SpecialistTask

    lowered = text.lower()
    specialist_id = next(
        (sid for sid, keywords in _SPECIALIST_KEYWORDS.items() if any(k in lowered for k in keywords)),
        None,
    )
    if specialist_id is None:
        return (
            "Which specialist? Try \"ask the qa specialist ...\", \"commercial "
            "specialist ...\", \"documentation specialist ...\", or \"support specialist ...\"."
        )

    cleaned = _clean_agent_request(text)
    for keywords in _SPECIALIST_KEYWORDS.values():
        cleaned = _remove_trigger(cleaned, list(keywords))

    manifest = REGISTRY.get(specialist_id)
    task = SpecialistTask(
        task_id=str(uuid.uuid4()),
        parent_objective_id="",
        specialist_id=specialist_id,
        task_type="LIGHT",
        objective=cleaned or text,
        scope=(manifest.allowed_domains[0] if manifest and manifest.allowed_domains else ""),
    )
    result = run_grounded_specialist(task)
...
def _handle_find_customers(text: str) -> str:
    """Real web search (Tavily), pre-seeded with NITE Submit's documented
    target audience when no specific query is given. See
    thursday/ops/research_ops.py's module docstring for why this is real
    search results, never fabricated leads, and why there's no internal
    CRM to fall back to (leads/enquiries/clients tables are real but
    empty).
    """
    import thursday.ops.research_ops as research_ops

    query = research_ops.parse_find_customers_command(text) or ""
    return research_ops.find_potential_customers(query)


def _handle_web_search(text: str) -> str:
    """General-purpose real web search (Tavily) for market/competitor
    research -- not customer-specific. See find_customers for the
    prospecting-flavoured variant.
    """
    import thursday.ops.research_ops as research_ops

    query = research_ops.parse_web_search_command(text)
    if not query:
        return "What should I search for? Try \"web search: <query>\"."
    result = research_ops.web_search(query)
    return research_ops.render_search_results(query, result)


def _handle_search_docs(text: str) -> str:
    """Real semantic search (local ONNX MiniLM embeddings, no network
    call) over docs/'s real markdown files. See
    thursday/ops/doc_search_ops.py's module docstring for the honest-
    degradation contract when the index or embedding model isn't
    available.
    """
    import thursday.ops.doc_search_ops as doc_search_ops

    query = doc_search_ops.parse_search_docs_command(text)
    if not query:
        return "What should I search the docs for? Try \"search docs: <query>\"."
    result = doc_search_ops.search_docs(query)
    return doc_search_ops.render_doc_search_results(query, result)


def _handle_reindex_docs(text: str) -> str:
    """Explicit, on-demand rebuild of the doc search index -- not run
    automatically on every search (~144 files is a real, non-trivial cost
    to redo per-request). See doc_search_ops.build_index().
    """
    import thursday.ops.doc_search_ops as doc_search_ops

    result = doc_search_ops.build_index()
    return doc_search_ops.render_reindex_result(result)


def _handle_notify_me(text: str) -> str:
    """Manual/ad-hoc push notification via ntfy -- see
    thursday/ops/notify_ops.py's module docstring. Mostly useful for
    testing the push channel; the real value is register_alert_push_handler()
    subscribing monitor.py's existing warning/critical alerts to the same
    channel automatically.
    """
    import thursday.ops.notify_ops as notify_ops

    message = notify_ops.parse_notify_me_command(text)
    if not message:
        return "What should the notification say? Try \"notify me: <message>\"."
    result = notify_ops.send_push_notification("Thursday", message)
    return notify_ops.render_notify_result(result)
