...
to surface bounded session-health advisories.  It does not have audio-rate
track signals, so name/fader overlap is reported as a candidate for an audio
check, never as measured masking.
"""
...
def _audit_loop() -> None:
    ...
    while True:
        ...
        try:
            ...
            state = live_client.query_session_state(include_meters=True)
            with _lock:
...
def _run_session_audits(tracks: List[Dict[str, Any]]) -> None:
    ...
    for idx, track in enumerate(tracks):
        ...
        meter_values = [
            float(value)
            for value in (track.get("output_meter_level"), track.get("output_meter_right"))
            if isinstance(value, (int, float)) and not isinstance(value, bool)
        ]
        if meter_values and max(meter_values) >= _NEAR_CEILING_METER:
            alert_id = f"meter-headroom-{idx}"
            peak_meter = max(meter_values)
            new_alerts[alert_id] = {
                "id": alert_id,
                "type": "headroom",
                "track_index": idx,
                ...
                "severity": "warning",
                "message": (
                    f"Track '{name}' latest output-meter reading is {peak_meter:.2f} "
                    "on Ableton's 0.0-1.0 scale, near the meter ceiling."
                ),
                "fix_action": (
                    f"Play the loudest section and inspect track {idx} with a true-peak meter "
                    "before changing gain or limiting."
                ),
                "fix_label": "Inspect Track Headroom",
            }
        ...
        # Collect likely low-end sources for an overlap *candidate*.  This
        # poll has names, mute state and faders, not audio-rate spectra or
        # time-aligned stems.  It must not claim that masking was measured.
        if not bool(track.get("muted")) and any(term in name_lower for term in {"kick", "sub", "bass", "low"}):
            if vol > 0.75:
                low_end_tracks.append((idx, name, vol))

    # 3. Low-end overlap candidate: multiple likely low-end sources are
    # unmuted and high on the faders.  Actual masking requires isolated or
    # time-aligned audio analysis and is provided by Mix Review's stem path.
    if len(low_end_tracks) >= 2:
        for i in range(len(low_end_tracks)):
            for j in range(i + 1, len(low_end_tracks)):
                ...
                alert_id = f"low-end-overlap-{idx1}-{idx2}"
                new_alerts[alert_id] = {
                    "id": alert_id,
                    "type": "low_end_overlap_candidate",
                    "track_indices": [idx1, idx2],
                    "severity": "info",
                    "message": (
                        f"Likely low-end overlap candidates: '{name1}' ({vol1:.2f}) and "
                        f"'{name2}' ({vol2:.2f}) are unmuted and high on the faders. "
                        "This is not an audio masking measurement."
                    ),
                    "fix_action": (
                        f"Inspect tracks {idx1} and {idx2} in Mix Review or audition them "
                        "together in mono before changing EQ or level."
                    ),
                    "fix_label": "Inspect Low-End Relationship",
                }
