...
def _row(command: str, *, llm_status: str, comparison_status: str | None, deterministic: str, llm_action: str) -> dict:
    llm = {
        "mode": "shadow",
        "status": llm_status,
    ...
    return {"response": {"command": command, "llm": llm}}


def test_shadow_report_counts_acceptance_matches_divergence_and_novel_phrasing() -> None:
    report = analyze([
        _row("mute the bass", llm_status="accepted", comparison_status="match", deterministic="set_mute", llm_action="set_mute"),
        _row("tuck the vocal a touch", llm_status="accepted", comparison_status="mismatch", deterministic="clarify", llm_action="set_volume"),
