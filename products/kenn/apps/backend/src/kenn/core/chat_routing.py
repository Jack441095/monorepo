...
from kenn.core.suggestions import DEFAULT_STARTERS, starter_questions
...
def is_unclear_query(query: str) -> bool:
    ...
    if re.search(r"\bableton\b", query, re.IGNORECASE):
        return False
    if re.search(r"^how\s+(do|can|to|should)\b", cleaned):
        return False
...
def _looks_like_audio_generation_request(query: str) -> bool:
    """Cheap local guard before importing the optional AudioGen provider."""
    lowered = query.lower()
    if re.search(r"^how\s+(do|can|to|should)\b", lowered):
        return False
    music_objects = (
        "arrangement", "beat", "chorus", "composition", "instrumental", "loop",
        "melody", "music", "phrase", "song", "track",
    )
    if not any(re.search(rf"\b{re.escape(obj)}\b", lowered) for obj in music_objects):
        return False
    if re.search(r"\b(generate|compose|render)\b", lowered):
        return True
    if any(
        re.search(
            rf"\b{verb}\s+(?:me\s+)?(?:a|an|the|some|another)\s+[^?.]{{0,40}}\b{obj}\b",
            lowered,
        )
    ...
    return bool(
        re.search(
            r"\btoo\s+repetitive\b|\b(?:give|try|generate)\s+(?:me\s+)?(?:a\s+)?"
            r"(?:different|another|new)\s+(?:variation|version|take|idea)\b",
            lowered,
        )
    )


def _audio_generation_unavailable_payload(query: str) -> dict:
    return {
        "question": query,
        "answer": (
            "Audio generation is unavailable in this beta. I have not generated or changed any audio.\n\n"
            "You can still ask me to create a production brief, suggest an arrangement, or propose "
            "a confirmation-gated MIDI workflow."
        ),
        "sources": [],
        "found": False,
        "confidence": "high",
        ...
        "related_questions": [
            "Help me write a production brief",
            "Suggest an arrangement for my track",
            "What MIDI workflow can KENN propose?",
        ],
        "used_history": False,
        "llm_enhanced": False,
        "llm_available": False,
        "intent": "generate",
        "route": "audiogen_unavailable",
        "intent_guard": "not_needed",
        "weak_match": False,
        "grounding_mode": "not_applicable",
        "diagnostic_reason": "Optional AudioGen provider is not installed in this beta.",
        "conversation_only": False,
        "branches": [],
        "session": None,
    }
...
def audio_generation_payload(query: str, *, session_id: str = "") -> dict | None:
    if set(query_topics(query)) & GAME_ROUTE_TOPICS:
        return None
    # Avoid importing and warning about an intentionally optional provider
    # on every ordinary knowledge request. This also gives real generation
    # requests an honest beta-boundary response when the provider is absent.
    if not _looks_like_audio_generation_request(query):
        return None
    if str(WEBSITE_ROOT) not in sys.path:
        sys.path.insert(0, str(WEBSITE_ROOT))
    try:
        ...
        return _audio_generation_unavailable_payload(query)
    ...
    # reversible proposal and verified readback path. Generation must not
    # create an implicit Ableton mutation as a side effect.
    load_message = ""
    load_success = False
    ...
    if track_match and wav_path:
        ...
        load_message = (
            "\n- Automatic Ableton clip loading is disabled until an exact, typed, "
            "reversible clip-loading proposal exists."
        )
    ...
    return {
        ...
        "audiogen": generation,
        "ableton_load_status": "disabled" if track_match and wav_path else "not_requested",
    }
...
def route_query(query: str, history: list | None = None) -> str:
    ...
    # Fast-path: Audio engineering & studio dialogue patterns (sub-1ms dispatch)
    lowered = query.lower().strip()
    if any(re.search(p, lowered) for p in (
        r"\b(?:what is|how do i|how to|can you explain|why is|tell me about|tips for|best way to|difference between)\b",
        r"\b(?:eq|compression|reverb|delay|saturation|mastering|sidechain|mix|kick|snare|vocal|synth|limiter|headroom|clipping|stereo)\b",
    )):
        return "production"
...
def classify_answer_mode(query: str, route: str, history: list | None = None) -> str:
    ...
    if route == "mix_review_followup" or latest_mix_review_context(history):
        return "mix_review_followup"
    if route in {"conversation", "production_dialogue"} or conversational_intent(query):
        return "studio_dialogue"
    topics = set(query_topics(query))
    intent = detect_intent(query)
...
def mode_profile(answer_mode: str) -> dict[str, str]:
    profiles = {
        ...
        "studio_dialogue": {
            "check_label": "Studio check:",
            "check": "Audition the move in the context of the full mix, comparing at matched loudness.",
            "avoid": "Avoid dogmatic rules; balance technical acoustic limits with creative intent.",
        },
    }
