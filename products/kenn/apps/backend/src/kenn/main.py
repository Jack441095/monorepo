...
from kenn.paths import PRODUCT_ROOT, TOOLING_ROOT
...
def print_help() -> None:
    print(
        ...
        "Usage:\n"
        "  python main.py build [--include-local-manuals]\n"
...
def main() -> int:
    ...
    if command == "download-pdfs":
        return subprocess.run(
            [sys.executable, str(TOOLING_ROOT / "scripts" / "setup" / "download_training_pdfs.py"), *rest],
            cwd=REPO_ROOT,
            check=False,
        ).returncode
    if command == "build-checklist":
        return subprocess.run(
            [sys.executable, str(TOOLING_ROOT / "scripts" / "build_audio_too_checklist.py"), *rest],
            cwd=REPO_ROOT,
            check=False,
        ).returncode
