...
def _explicit_rating_label(rating: object) -> int | None:
    """Map KENN's documented 1–5 feedback scale to a binary signal.

    Ratings 4–5 mean helpful, 1–2 mean unhelpful, and 3 is deliberately
    neutral.  The old calibrator expected a legacy 1/-1 scale, so normal
    endpoint feedback was silently excluded from calibration.
    """
    try:
        value = int(rating)
...
def train_calibrator() -> dict:
    ...
    for row in rows:
        ...
        if rating is not None:
            label = _explicit_rating_label(rating)
        else:
