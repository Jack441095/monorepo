...
def normalize_session(session: dict[str, Any] | None) -> NormalizedProject:
    ...
    for position, raw in enumerate(session.get("tracks") or []):
        ...
        tracks.append(ProjectTrack(
            ...
            output_meter_level=_bounded_meter(raw.get("output_meter_level")),
            output_meter_right=_bounded_meter(raw.get("output_meter_right")),
            color=_optional_int(raw.get("color")),
            inferred_role=infer_track_role(name),
        ))
    has_instantaneous_track_meters = any(
        value is not None
        for track in tracks
        for value in (track.output_meter_level, track.output_meter_right)
    )
    available_capabilities = (
        "track_names", "mixer_state", "device_names", "device_active_state", "scenes",
        "tempo", "transport", "track_groups", "clip_presence", "returns", "master",
    )
    if has_instantaneous_track_meters:
        available_capabilities += ("instantaneous_track_meters",)
...
def recommendations_from_mix_review(review: dict[str, Any] | None) -> list[Recommendation]:
    ...
    for flag in review.get("flags") or []:
        ...
        raw_confidence = flag.get("confidence")
        if isinstance(raw_confidence, (int, float)):
            confidence = max(0.0, min(1.0, float(raw_confidence)))
        else:
            confidence_value = str(raw_confidence or "medium").lower()
    ...
    largest_ltas = comparison.get("largest_ltas_difference") if isinstance(comparison.get("largest_ltas_difference"), dict) else {}
    frequency = largest_ltas.get("center_hz")
    ltas_delta = largest_ltas.get("delta_db")
    if isinstance(frequency, (int, float)) and not isinstance(frequency, bool) and isinstance(ltas_delta, (int, float)) and not isinstance(ltas_delta, bool):
        direction = "more prominent" if float(ltas_delta) > 0 else "less prominent"
        recommendations.append(Recommendation(
            title=f"Check around {float(frequency):.0f} Hz against reference", category="reference", severity="info", confidence=0.85,
            description=(
                f"The uploaded 40-band LTAS comparison measures this mix as {abs(float(ltas_delta)):.1f} dB "
                f"{direction} around {float(frequency):.0f} Hz relative to the uploaded reference."
            ),
            reason=(
                "Each spectrum is normalised around 1 kHz and measured against the supplied reference, "
                "not a universal pink-noise or genre target."
            ),
            suggestedAction=(
                "Level-match equivalent sections, then audition low-mid masking and broad tonal balance. "
                "Use any EQ move as a partial starting point and bypass before keeping it."
            ),
        ))
    pink_reference = comparison.get("pink_noise_reference") if isinstance(comparison.get("pink_noise_reference"), dict) else {}
    pink_largest = pink_reference.get("largest_deviation") if isinstance(pink_reference.get("largest_deviation"), dict) else {}
    pink_frequency = pink_largest.get("center_hz")
    pink_deviation = pink_largest.get("deviation_db")
    pink_curve = str(pink_reference.get("curve") or "-3 dB/octave pink-noise-style baseline").strip()
    if (
        isinstance(pink_frequency, (int, float)) and not isinstance(pink_frequency, bool)
        and isinstance(pink_deviation, (int, float)) and not isinstance(pink_deviation, bool)
    ):
        pink_direction = "above" if float(pink_deviation) > 0 else "below"
        recommendations.append(Recommendation(
            title=f"Check around {float(pink_frequency):.0f} Hz against pink-noise reference",
            category="reference", severity="info", confidence=0.78,
            description=(
                f"The measured LTAS is {abs(float(pink_deviation)):.1f} dB {pink_direction} the explicit "
                f"{pink_curve} around {float(pink_frequency):.0f} Hz."
            ),
            reason=(
                "This is a broad spectral-shape reference for the uploaded render, not a universal target, "
                "quality score, or diagnosis of the responsible track or processing stage."
            ),
            suggestedAction=(
                "Level-match and listen in the full arrangement, then audition source-level changes one at a time; "
                "do not apply automatic master EQ from this measurement."
            ),
        ))
...
def prioritize_recommendations(
    recommendations: list[Recommendation], *, genre: str | None = None
) -> list[Recommendation]:
    """Rank measured Mix Review recommendations for a producer.

    Most severe and most confident first; exact duplicate findings (same
    category and title) are merged. An explicit ``genre`` -- supplied by the
    caller, never inferred from the audio -- appends one advisory reference
    note describing that genre's typical targets. It never changes a
    finding's severity, never claims the file passed or failed against those
    targets, and never fabricates a new measured finding.
    """
    ...
    ranked = sorted(
        unique,
        key=lambda item: (_SEVERITY_RANK.get(item.severity, len(_SEVERITY_RANK)), -item.confidence),
    )
    if genre:
        from kenn.core.genre_profiles import GENRE_PROFILES

        profile = GENRE_PROFILES.get(genre)
        if profile is not None:
            ranked.append(Recommendation(
                title=f"{profile.display_name} reference targets",
                category="genre_reference",
                severity="info",
                confidence=1.0,
                description=(
                    f"Typical {profile.display_name} targets: loudness around {profile.target_lufs:g} LUFS, "
                    f"peaks at or below {profile.max_peak_dbfs:g} dBFS, crest factor "
                    f"{profile.target_crest_range_db[0]:g}-{profile.target_crest_range_db[1]:g} dB, "
                    f"{profile.compression_character} compression character."
                ),
                reason="Reference context only, supplied explicitly by the caller, not inferred from the audio; it does not change or override any measured finding above.",
                suggestedAction="Compare informally by ear; this is not a calibrated pass/fail against the uploaded file.",
                requiresConfirmation=False,
            ))
    return ranked


def summarize_recommendations(recommendations: list[Recommendation]) -> str:
    """Turn a ranked recommendation list into plain-language, prioritized
    advice. Deterministic templates only -- no free-form generation -- and
    every claim stays traceable to the Recommendation it came from."""
    actionable = [item for item in recommendations if item.category != "genre_reference"]
    if not actionable:
        return "No measured issues were found in this review."
    lines: list[str] = []
    critical = [item for item in actionable if item.severity == "critical"]
    warning = [item for item in actionable if item.severity == "warning"]
    info = [item for item in actionable if item.severity == "info"]
    if critical:
        lines.append("Fix first:")
        lines.extend(f"  - {item.title}: {item.suggestedAction}" for item in critical)
    if warning:
        lines.append("Worth addressing:" if lines else "Worth addressing first:")
        lines.extend(f"  - {item.title}: {item.suggestedAction}" for item in warning)
    if info:
        lines.append("For reference:" if lines else "For reference:")
        lines.extend(f"  - {item.title}: {item.description}" for item in info)
    genre_note = next((item for item in recommendations if item.category == "genre_reference"), None)
    if genre_note:
        lines.append(genre_note.description)
    return "\n".join(lines)
...
def _bounded_meter(value: Any) -> float | None:
    """Accept only Ableton's documented instantaneous 0.0-1.0 meter scale."""
