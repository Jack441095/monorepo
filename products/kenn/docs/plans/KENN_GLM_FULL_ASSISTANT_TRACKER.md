...
- [x] **C2 Model bake-off** on this Mac: `qwen2.5:1.5b`, `qwen2.5:7b-instruct`, Qwen3.5-2B/4B, Phi-4-mini (accuracy, clarification, p95 latency, memory)
  ...
  - [ ] Grow the rule-based parser for relative dB, focus, sends, slang (volume 1/12 today)
    ...
    > Absolute dB without the word volume ("put the kick at minus 12 dB", "set the snare to -10 dB") done 2026-09-24, same device/send/pan guard: 124 cases **63.7%** (79), 0 wrong plans. "Select the Drum Bus" (track focus by name) is still not understood. Punctuation changes the parse: "Focus EQ Eight on track 5." proposes a track focus (the demo contract) but without the full stop a device focus; owner to decide which is right.
