...
- [ ] **G1 Owned knowledge base:** KENN-written device and workflow notes; MiniLM ONNX fetched; hybrid retrieval
  ...
  > 2026-09-24: 21 draft device notes (7 uncovered + 14 mention-only devices, incl. EQ Eight with Live's measured parameter layout, Operator, Simpler, Sampler) drafted on the GPU 1 notes model from the Live 12 manual (`tooling/scripts/draft_device_notes_gpu.py`), Reviewed against the manual (6 unsupported lines removed) and approved the same day under owner delegation; index rebuilt and promoted (, retrieval fixture unchanged at recall@4 0.966). Coverage: 74 own, 2 mentioned, 2 none. The notes and index are git-ignored data; final copies also on the GPU box. See `docs/evidence/KENN_DEVICE_COVERAGE_2026-09-24.md`.
