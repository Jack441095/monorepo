...
The real mode reads KENN's local session endpoint, which owns the single
AbletonOSC reply socket. This keeps a qualification probe from competing with
the running companion for UDP 11001. Mock output is explicitly labelled and
cannot be used as real-Live evidence.
"""
...
def qualify(mode: str = "real", endpoint: str = "http://127.0.0.1:8090") -> dict[str, Any]:
    ...
    try:
        ...
        snapshot = {"status": "offline", "tracks": []}
    ...
    identity_ok = all(isinstance(track, dict) and "index" in track and "name" in track for track in tracks)
    checks = {
        "connected": usable,
