...
final class KENNDesktopCompanion: NSObject, NSApplicationDelegate, WKNavigationDelegate, WKUIDelegate {
    ...
    private func isRepository(_ url: URL) -> Bool {
        FileManager.default.fileExists(atPath: url.appendingPathComponent("studio/kenn/kenn/server.py").path)
    }
    ...
    private func launchServer() {
        ...
        guard let root = repositoryRoot else {
            setStatus("Choose the Audio_Too repository to start KENN", image: "exclamationmark.triangle.fill")
            return
        }
        let python = pythonExecutable(in: root)
        let task = Process()
        task.executableURL = URL(fileURLWithPath: python)
        task.arguments = [root.appendingPathComponent("studio/kenn/kenn/server.py").path]
        task.currentDirectoryURL = root
        var environment = ProcessInfo.processInfo.environment
        let paths = [root.path, root.appendingPathComponent("studio/kenn").path]
        environment["PYTHONPATH"] = paths.joined(separator: ":") + ":" + (environment["PYTHONPATH"] ?? "")
    ...
    private func ensureAutomixWorker() {
        ...
        task.arguments = [root.appendingPathComponent("main.py").path, "worker", "kenn-desktop"]
        task.currentDirectoryURL = root
        var environment = ProcessInfo.processInfo.environment
        environment["PYTHONPATH"] = [root.path, root.appendingPathComponent("studio/kenn").path].joined(separator: ":")
        task.environment = environment
