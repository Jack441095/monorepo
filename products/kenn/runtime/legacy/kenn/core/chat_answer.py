...
def format_source_as_link(chunk: dict) -> str:
    if chunk.get("kind") == "note":
        ...
        if title.lower().endswith(".md"):
            title = title[:-3]
        return f"[{title}](/api/ableton/note?name={filename})"
...
def _progressive_token_yield(
    ...
    # Yield each section with a pause
    first = True
    for section in sections:
        if not section.strip():
            continue
        if first:
            # First section yields immediately (the opener)
            yield {"event": "token", "token": section}
            first = False
        else:
            # Subsequent sections get a micro-pause (0.08-0.25s) for the "thinking" feel
            pause = 0.08 + (random.random() * 0.17)
            time.sleep(pause)
