"""Versioned wire contracts used by KENN's HTTP response envelope.

Ported into this standalone repository (not a live dependency on the old
NITE DSP platform): these dataclasses deliberately use only the Python
standard library, define wire-safe data rather than domain behavior, and
have no coupling to any other product. KENN's own response_contract.py and
endpoint_policy.py are the only consumers in this repo.

Each contract validates on construction and round-trips through ordinary
JSON dictionaries.
"""
...
class AssistantResponse:
    ...
    Core routing, grounding, sources, and confirmation state remain stable
    across HTTP, CLI, and other interfaces; interface-specific presentation
