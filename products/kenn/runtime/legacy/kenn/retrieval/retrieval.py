...
from kenn.retrieval.index_store import active_artifact_path, active_version_id, promote_index
...
def save_embedding_index(embeddings: np.ndarray) -> None:
    """Promote embeddings together with their matching chunks and terms."""
    chunks_path = active_artifact_path("chunks.jsonl", INDEX_DIR)
    ...
    promote_index(chunks, terms, embeddings, index_dir=INDEX_DIR)
    global _embedding_index, _embedding_index_version
...
TOPIC_SYNONYMS: dict[str, tuple[str, ...]] = {
    ...
    "automation": ("automation", "automate", "envelope", "lfo", "modulation", "macro", "clip envelope"),
    ...
    "routing": ("route", "routing", "group", "bus", "send", "return", "track"),
    ...
    "game_audio": ("game audio", "game", "interactive", "wwise", "fmod", "unity", "unreal", "soundbank", "sound bank", "soundbanks", "implementation", "runtime", "stinger", "interactive music", "ambience loop", "game build", "combat", "combat audio", "mobile build", "footstep", "footsteps"),
    "wwise": ("wwise", "audiokinetic", "rtpc", "game parameter", "game sync", "event", "events", "state", "states", "switch", "switches", "soundbank", "soundbanks", "sound bank", "game object", "middleware", "unity", "unreal", "profiling", "profiler", "voice count", "voices", "virtual voice", "memory", "silent", "no sound", "drop out", "drops out", "missing media", "setrtpcvalue", "bank load", "listener", "surface", "material", "transition segment", "music segment", "conversion", "sample-rate", "sample rate"),
...
def bm25_search(
    query: str, chunks: list[dict], terms: dict, limit: int = 8, *, rerank: bool = True
) -> list[tuple[float, dict]]:
    query_terms = expanded_query_terms(query)
    if not query_terms:
        return []
