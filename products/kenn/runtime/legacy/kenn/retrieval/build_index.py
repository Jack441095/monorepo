...
def build_manifest_metadata(indexed_files: list[tuple[Path, str]]) -> dict:
...
def should_index_pdf(catalog_entry: dict | None) -> bool:
    entry = catalog_entry or {}
    return str(entry.get("index_policy", "index")).strip().lower() != "reference_only"
...
def build_index(pdf_dir: Path = PDF_DIR, notes_dir: Path = NOTES_DIR) -> list[Chunk]:
    import os
    ...
    version_id = promote_index(
        ...
        embeddings,
        index_dir=INDEX_DIR,
        manifest_metadata=build_manifest_metadata(indexed_files),
    )
    ...
    print(f"PDFs:   {len(pdfs) - skipped_pdfs} indexed, {skipped_pdfs} reference-only")
...
def main() -> int:
    ...
    build_index(args.pdf_dir, args.notes_dir)
    try:
