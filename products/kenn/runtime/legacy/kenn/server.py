...
# ROOT is <repo>/source/kenn, PROJECT_ROOT is <repo>/source, so the repo
# root is one level up from PROJECT_ROOT. A prior monorepo-era build used
# two levels up, which pointed one directory *above* this standalone repo
# -- see docs/KNOWN_ISSUES.md.
REPO_ROOT = PROJECT_ROOT.parent

# Load .env from the repo root so action-policy flags (AUDIO_TOO_ALLOW_DAW_CONTROL
# etc.) are available regardless of how the server is launched.
...
_VT_ROOT = REPO_ROOT / "UX" / "velvet_thunder"
WEBSITE_ROOT = (_VT_ROOT / "business" / "app") if (_VT_ROOT / "business" / "app").exists() else (REPO_ROOT / "business" / "app")
ANALYSIS_TOOL_ROOT = (_VT_ROOT / "studio" / "audio_analysis") if (_VT_ROOT / "studio" / "audio_analysis").exists() else (REPO_ROOT / "studio" / "audio_analysis")
# Served UI lives at <repo>/UX/ (a colleague's redesign of the chat/
# dashboard front-end, integrated 2026-09-01 -- see docs/KNOWN_ISSUES.md).
STATIC_ROOT = REPO_ROOT / "UX"
PORTFOLIO_AUDIO_ROOT = REPO_ROOT / "business" / "portfolio" / "audio"
_PORTFOLIO_CANDIDATE = _VT_ROOT / "business" / "portfolio" / "audio"
PORTFOLIO_AUDIO_ROOT = _PORTFOLIO_CANDIDATE if _PORTFOLIO_CANDIDATE.exists() else (REPO_ROOT / "business" / "portfolio" / "audio")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/health":
            ...
            try:
                index_pointer = REPO_ROOT / "source" / "kenn" / "data" / "index" / "CURRENT"
...
def main() -> int:
    ...
    setup_server_logging("kenn", ROOT / "logs")
