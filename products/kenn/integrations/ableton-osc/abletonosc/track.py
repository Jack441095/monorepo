...
from .browser_search import find_browser_device_item
from .browser_sample_search import find_sample_item_by_path
...
class TrackHandler(AbletonOSCHandler):
    ...
    def init_api(self):
        ...
        # ``group_track`` is a Live object reference, so expose its stable
        # index in the song's regular-track collection instead of passing the
        # object through the generic OSC property callback. ``None`` means
        # this track is not a member of another group.
        def track_get_group_track(track, params: Tuple[Any] = ()):
            if track.group_track is None:
                return (None,)
            return (list(self.song.tracks).index(track.group_track),)

        self.osc_server.add_handler("/live/track/get/group_track", create_track_callback(track_get_group_track))
        properties_rw = [
            "arm",
            "color",
            "color_index",
        ...
        def track_insert_device(track, params: Tuple[Any]):
            ...
            # Search the complete browser tree. Live exposes the built-in
            # effects below nested category BrowserItems, so looking only at
            # the first-level children misses native devices such as EQ Eight.
            search_locations = [
            ...
            # A category can share a display name with its loadable leaf
            # (notably some native effects). Never select a category for
            # load_item: it can raise inside Live and leave the OSC caller
            # waiting with no reply. The search itself (exact match anywhere
            # in the tree beats any substring match) lives in
            # browser_search.py so it is unit-testable without Live.
            device_item = find_browser_device_item(search_locations, device_uri)

            if device_item:
                # Browser loading targets Live's selected track rather than
                # the track object supplied by this callback. Select the
                # requested track first so a visible KENN device window or a
                # different user selection cannot redirect the insertion.
                self.song.view.selected_track = track
                before_count = len(track.devices)
                try:
                    browser.load_item(device_item)
                except Exception as exc:
                    self.logger.exception("Could not load device '%s' on track '%s': %s", device_uri, track.name, exc)
                    return (-1,)
                after_count = len(track.devices)
                if after_count <= before_count:
                    self.logger.warning("Live did not add device '%s' to track '%s'" % (device_uri, track.name))
                    return (-1,)
        ...
        #--------------------------------------------------------------------------------
        # Import one real sample file from disk into one exact, empty clip
        # slot (Stage 3 item 3 -- see docs/ABLETON_ASSISTANT_CURRENT_STATE.md's
        # 2026-09-06 research entry). Only resolves a file that lives inside
        # a folder the user has already registered as a Place in Ableton's
        # own Library preferences: real testing found the Remote Script's
        # Browser has no `add_item_to_places` to register one programmatically
        # (that method exists only on Push 3's separate, unrelated hardware
        # browser API). Usage: /live/track/import_sample <track_index>
        # <clip_slot_index> <absolute_path>
        #--------------------------------------------------------------------------------
        def track_import_sample(track, params: Tuple[Any]):
            track_index, clip_slot_index, absolute_path = params
            clip_slot_index = int(clip_slot_index)
            absolute_path = str(absolute_path)
            if clip_slot_index < 0 or clip_slot_index >= len(track.clip_slots):
                return (clip_slot_index, False, "Clip-slot index is out of range for this track.")
            slot = track.clip_slots[clip_slot_index]
            if slot.has_clip:
                return (clip_slot_index, False, "Target clip slot already contains a clip; import refused.")

            application = Live.Application.get_application()
            browser = application.browser
            _debug_stats: dict = {}
            try:
                item = find_sample_item_by_path(list(browser.user_folders), absolute_path, stats=_debug_stats)
            except Exception as exc:
                return (clip_slot_index, False, "Browser search failed: %s" % exc)
            if item is None:
                # Logged, not returned to the caller: real testing (2026-09-06)
                # found search completeness against a large personal library is
                # not yet fully characterized (see docs/ABLETON_ASSISTANT_CURRENT_STATE.md) --
                # this stays internal rather than leaking raw counters into a
                # user-facing error message.
                self.logger.info("import_sample: no match for %s (%s)" % (absolute_path, _debug_stats))
                return (
                    clip_slot_index,
                    False,
            ...
            if not fresh_slot.has_clip:
                return (clip_slot_index, False, "Import did not create a clip.")
            try:
                clip_name = str(fresh_slot.clip.name)
            except Exception:
                clip_name = ""
            return (clip_slot_index, True, clip_name)
