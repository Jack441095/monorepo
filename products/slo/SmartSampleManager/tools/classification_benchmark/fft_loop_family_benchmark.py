#!/usr/bin/env python3
"""Collection-grouped FFT specialist ablation for loop families.

This is the targeted follow-up to ``fft_sidecar_benchmark.py``. It asks a
narrow question: do the production sidecar features add information to the
existing full-file loop spectrum for Drum/Percussion/Hi-Hat/Foley loop
families? It is read-only research and never creates rename actions.
"""
...
import loop_features
from fft_sidecar_benchmark import FEATURE_NAMES as FFT_NAMES
from fft_sidecar_benchmark import extract as extract_fft


LOOP_NAMES = list(loop_features.NAMES)
ALLOWED = {"Drum Loop", "Percussion Loop", "Hi-Hat Loop", "Foley Loop",
           "Chord Loop", "Top Loop", "Bass Loop"}


def _extract_one(path: str):
    return path, extract_fft(path), loop_features.extract(path)


def _load_rows(labels: Path):
    out = []
...
def _score(x, y, groups, seed):
    ...
    return {
        ...
        "macro_f1": round(float(100.0 * f1_score(y, pred, average="macro",
                                                   zero_division=0)), 3),
    }
...
def main() -> int:
    ...
    if args.cache.exists():
        ...
        if list(z["fft_names"].astype(str)) == FFT_NAMES and list(z["loop_names"].astype(str)) == LOOP_NAMES:
            cached = {os.path.abspath(str(p)): (z["fft"][i], z["loop"][i])
                      for i, p in enumerate(z["paths"])}
    todo = [path for path, _ in rows if path not in cached]
    if todo:
        with mp.Pool(max(1, args.workers)) as pool:
            for n, (path, fft, loop) in enumerate(pool.imap_unordered(_extract_one, todo), 1):
                if fft is not None and loop is not None:
                    cached[path] = (fft, loop)
                if n % 50 == 0:
                    print(f"extracted {n}/{len(todo)}", flush=True)

    rows = [(path, label) for path, label in rows if path in cached]
    # Require enough examples and independent collections for every class;
    # otherwise a family would make the fold protocol ill-posed.
    inventory = {}
    ...
    viable = [label for label, stats in inventory.items()
              if stats["rows"] >= 5 and stats["collections"] >= 5]
    ...
    loop = np.asarray([cached[p][1] for p in paths], dtype=np.float32)
    groups = np.asarray([os.path.dirname(os.path.dirname(p)) for p in paths], dtype=object)

    args.cache.parent.mkdir(parents=True, exist_ok=True)
    np.savez(args.cache, paths=paths, fft=fft, loop=loop,
             fft_names=np.asarray(FFT_NAMES, dtype=object),
             loop_names=np.asarray(LOOP_NAMES, dtype=object))

    arms = {"loop_spectrum": loop, "fft_sidecar": fft,
            "loop_spectrum_plus_fft": np.hstack([loop, fft])}
    ...
    receipt = {
        "record_type": "slo_fft_loop_family_benchmark",
        "schema_version": "1.0.0",
        "generated": time.strftime("%Y-%m-%dT%H:%M:%S"),
        "protocol": {"split": "StratifiedGroupKFold by two-level collection",
                     "seeds": args.seeds, "folds": 5, "model": "balanced RF, 300 trees",
        ...
        "inputs": {"labels": str(args.labels.resolve()), "rows": int(len(y)),
                   ...
                   "inventory": inventory},
        "features": {"fft_sidecar": FFT_NAMES, "loop_spectrum": LOOP_NAMES},
        "results": summary,
