...
def export_v3_to_cpp(checkpoint_path, header_path):
    ...
    with open(header_path, "w") as f:
        f.write("#pragma once\n\n")
        f.write("// Automated weights export for Nite DSP SLO AcousticClassifier V3 (ArcFace / Gated ResNet).\n")
        f.write("// Trained on all 33 sample packs (21,793 samples across 16 categories).\n")
        f.write("// DO NOT EDIT MANUALLY.\n\n")
        f.write("namespace AcousticWeights\n{\n")
        f.write("    constexpr int modelVersion = 3;\n")
        ...
        # Arc Head weights (normalized cosine similarity weights)
        write_matrix_2d("arc_head_weights", state_dict["arc_head.weight"])
        
        f.write("}\n")

    print(f"Successfully exported C++ header to {header_path}")

if __name__ == "__main__":
    ckpt = "/Volumes/Jack_Gandy_1TB_SSD/NITE_DSP/products/slo/SmartSampleManager/tools/classification_benchmark/slo_classifier_v3_all_packs.pt"
    out_header = "/Volumes/Jack_Gandy_1TB_SSD/NITE_DSP/products/slo/SmartSampleManager/Source/AcousticClassifierWeights.h"
    export_v3_to_cpp(ckpt, out_header)
